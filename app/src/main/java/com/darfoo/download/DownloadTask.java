...
import com.darfoo.download.error.NoMemoryException;
import com.darfoo.download.utils.AndroidHttpClient;
import com.darfoo.download.utils.NetworkUtils;
...
public class DownloadTask extends AsyncTask<Void, Integer, Long> {
    ...
    public DownloadTask(Context context, String url, String path, String filename, FileDetail detail) throws MalformedURLException {

        this(context, url, path, filename, detail, null);
    }

    public DownloadTask(Context context, String url, String path, String filename, FileDetail detail, DownloadTaskListener listener)
        ...
        this.detail = detail;
        Log.d("DownloadTask", this.file.getPath());
        Log.d("DownloadTask", url);
        Log.d("DownloadTask", path);
    ...
    private long download() throws NetworkErrorException, IOException, FileAlreadyExistException,
        ...
        if (!NetworkUtils.isNetworkAvailable(context)) {
            throw new NetworkErrorException("网络不佳，请重试。");
        }
        ...
        if (file.exists() && totalSize == file.length()) {
            if (DEBUG) {
                Log.v(null, "视频/音乐已存在，请到下载或管理页面查看");
            }

            throw new FileAlreadyExistException("视频/音乐已存在，请到下载或管理页面查看。");
        ...
        if (totalSize - tempFile.length() > storage) {
            throw new NoMemoryException("内存空间不足");
        }
        ...
        if ((previousFileSize + bytesCopied) != totalSize && totalSize != -1 && !interrupt) {
            throw new IOException("下载不完全: " + bytesCopied + " != " + totalSize);
        }

        if (DEBUG && !interrupt) {
            Log.v(TAG, "下载成功。");
        } else if(DEBUG && interrupt) {
            Log.v(TAG, "下载暂停。");
        }
                ...
                if (networkSpeed == 0) {
                    if (errorBlockTimePreviousTime > 0) {
                        ...
                        if (expireTime > TIME_OUT) {
                            throw new ConnectTimeoutException("网络连接超时，请重试。");
                        }
                    } else {
